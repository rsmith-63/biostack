# Refactored BLAST API Client (Node v24.11.0)

## 1. `blast.js` (Refactored without the `Blast` class)
...
const BLAST_BASE_URL = '[https://blast.ncbi.nlm.nih.gov/blast/Blast.cgi](https://blast.ncbi.nlm.nih.gov/blast/Blast.cgi)';
...
export const createBlastClient = (config = {}) => {
  ...
  const retrieve = async (rid) => {
    ...
    const responseText = await fetchText(url, pollLimiter);
    return mapBlastResult(JSON.parse(responseText));
  };
